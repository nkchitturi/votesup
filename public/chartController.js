...
voteSUpChartController = function() {
    ...
    function incrementVoteViaVoteCounts(colorToInc) {
        var incUrl = apiBaseurl + 'addVote?color=' + colorToInc;
        $.getJSON(incUrl, {}, function(data, status) {
            ...
            if (status !== 'success') {
                console.log('Failed to fetch /addVote?color=' + colorToInc);
            } else if (data.hasOwnProperty('error')) {
                console.log('/addVote error: ' + data.error);
                ...
                for (segmentIndex in voteSUpDoughnutChart.segments) {
                    ...
                    if (segmentColor === colorToInc) {
                        ...
                        updateChart = true;
                        updateLastApiMessage('Added ' + colorToInc +
                            ' ... new vote count is ' + data.count);
                    }
                }
            }
        });
    ...
    function pollForUpdates() {
        ...
        $.getJSON(apiBaseurl + 'voteData?voteCountsOnly=true', {}, function(data, status) {
            var segment;
            var segmentIndex;
            var color;
            var doUpdate = false;
            ...
            if (status !== 'success') {
                console.log('Failed to fetch /voteData?voteCountsOnly=true');
                return;
            }
    ...
    $.getJSON('rootPoll.json', {}, function(data, status) {
        ...
        $.getJSON(apiBaseurl + 'voteData', {}, function(data, status) {
            var i;

            if (status !== 'success') {
                console.log('Failed to fetch /voteData');
                return;
            }
    ...
    $('#colorCounts').click(function(evt) {
        ...
        if (colorMatch !== null) {
            incrementVoteViaVoteCounts(colorMatch[1]);
        }
    });
    ...
    $('#voteSUpChart').click(function(evt) {
        ...
        incUrl = apiBaseurl + 'addVote?color=' + colorToInc;
        $.getJSON(incUrl, {}, function(data, status) {
